...
// Words to skip during title capital protection.
// These are typically lowercase in titles even when surrounding words are capitalized.
const TITLE_STOP_WORDS = new Set([
  'a', 'an', 'the', 'and', 'but', 'or', 'for', 'nor',
  'of', 'in', 'on', 'at', 'to', 'by', 'with', 'from', 'as', 'is',
]);

/**
 * Protect title capitalization by wrapping the first character of each
 * significant word in braces: {W}ord. This prevents BibTeX styles that
 * apply \MakeLowercase from destroying intended capitalization.
 *
 * Rules:
 *  - Always protect the first word of the title (even if it's a stop word).
 *  - Always protect the first word after a colon (:).
 *  - Skip stop words (articles, prepositions, conjunctions).
 *  - Skip words that already have braces around their first character.
 *  - Only wrap the first character if it is already uppercase.
 */
function protectTitleCapitals(title: string): string {
  // Split into tokens: words, whitespace, and colons (preserve all exactly)
  ...
  return tokens.map(token => {
    ...
    // Colon — set flag for next word, pass through
    if (token === ':') {
      afterColon = true;
      return token;
    }
    ...
    // Only protect if the first character is uppercase
    if (/^[A-Z]/.test(token)) {
      const stripped = token.toLowerCase().replace(/[^a-z]/g, '');
      const isStopWord = TITLE_STOP_WORDS.has(stripped);
      const shouldProtect = isFirstWord || afterColon || !isStopWord;
...
export function sanitizeBibField(value: string, fieldName: string): string {
  ...
  // Title/booktitle: protect capitalization from \MakeLowercase in citation styles.
  // Strategy: wrap the first character of each significant word in braces.
  // Stop words (articles, prepositions, conjunctions) are left unprotected so
  // citation styles can lowercase them.  The first word of the title and the
  // first word after a colon are always protected regardless.
  // Also escape & and % which are always dangerous.
  if (fieldName === 'title' || fieldName === 'booktitle') {
    let escaped = v
      .replace(/(?<!\\)&/g, '\\&')
      .replace(/(?<!\\)%(?!\d)/g, '\\%');
    escaped = protectTitleCapitals(escaped);
    return escaped;
  }
  ...
  // URL: verbatim in BibTeX — no escaping needed (url package treats contents literally)
  if (fieldName === 'url') {
    return v;
  }

  // DOI/eprint: verbatim in BibTeX — no escaping needed
  if (fieldName === 'doi') {
    return v.replace(/^https?:\/\/doi\.org\//i, '');
  }
  if (fieldName === 'eprint') {
    return v;
  }
...
// ============================================================
// DOI Structure Inference — Auto-fill missing fields from DOI
// No network call needed. DOI structure encodes publisher and year.
// ============================================================

/**
 * Infer missing bibliography fields from DOI structure alone.
 * DOI format: 10.{registrant}/{suffix}
 * Common registrant prefixes reveal publisher, year, and type.
 *
 * @example
 * inferFromDOI('10.1109/CVPR.2023.00456')
 * // => { year: '2023', publisher: 'IEEE', type: 'inproceedings', booktitle: 'CVPR' }
 */
export function inferFromDOI(doi: string): Partial<Record<string, string>> {
  if (!doi?.startsWith('10.')) return {};

  const afterPrefix = doi.slice(3);
  const [registrant] = afterPrefix.split('/');
  const inferred: Partial<Record<string, string>> = {};

  // Year in suffix pattern: 10.1109/CVPR.2023.00456 → year 2023
  const yearInSuffix = doi.match(/\.(20\d{2}|19\d{2})\./);
  if (yearInSuffix) inferred.year = yearInSuffix[1];

  // Publisher inference from registrant code
  const REGISTRANT_MAP: Record<string, string> = {
    '1145':  'ACM',
    '1109':  'IEEE',
    '1007':  'Springer',
    '1016':  'Elsevier',
  ...
  const publisher = REGISTRANT_MAP[registrant];
  if (publisher && !inferred.publisher) inferred.publisher = publisher;

  // IEEE DOIs often contain conference abbreviation
  if (registrant === '1109' || registrant === '1061' || registrant === '1103') {
    const confMatch = doi.match(/10\.\d+\/([A-Z]{2,}(?:\d|\/)[A-Z]*)\./);
    if (confMatch && confMatch[1].length >= 2 && confMatch[1].length <= 10) {
      const confName = confMatch[1];
      // Common IEEE conference codes
      const knownConferences = ['CVPR', 'ICCV', 'ECCV', 'NeurIPS', 'ICML', 'ICLR',
        'AAAI', 'IJCAI', 'ACL', 'EMNLP', 'NAACL', 'SIGCHI', 'SIGIR', 'KDD',
        'WWW', 'RSS', 'ICRA', 'IROS', 'CDC', 'ACC', 'GLOBECOM', 'INFOCOM',
        'ISCAS', 'MWSCAS', 'ICC', 'GTC'];
      if (knownConferences.some(c => confName.startsWith(c))) {
        inferred.type = 'inproceedings';
        inferred.booktitle = confName;
      }
    }
  }

  // ACM DOIs may indicate conference proceedings
  if (registrant === '1145' || registrant === '2320') {
    const confMatch = doi.match(/10\.\d+\/([A-Z]+)\.\d{4}/);
    if (confMatch) {
      inferred.type = 'inproceedings';
      inferred.booktitle = confMatch[1];
    }
  }
...
export function generateBibEntry(
  ...
  // type is already a valid BibEntryType string, use directly
  return `@${type}{${key},\n${fields.join(',\n')}\n}`;
}
...
export function generateBibFromThesisReferences(references: ThesisReference[]): string {
  return generateBibFile(
    references.map(ref => {
      ...
      // 'thesis' maps to 'phdthesis' or 'mastersthesis' depending on degree level.
      // 'techreport' maps directly (same name in both type systems).
      const isMasterThesis = ref.type === 'thesis' && ref.degree && ref.degree.toLowerCase() !== 'phd';
      const bibType: BibEntryType =
        ref.type === 'thesis' ? (isMasterThesis ? 'mastersthesis' : 'phdthesis') :
      ...
      // Auto-populate the BibTeX "type" field for thesis and techreport entries.
      // This field is required by the phdthesis/mastersthesis/techreport schemas.
      // Without it, generateBibEntry outputs a TODO placeholder that breaks compilation.
      const bibTypeField = bibType === 'phdthesis' ? "Ph.D. dissertation" :
                           bibType === 'mastersthesis' ? "Master's thesis" :
                           bibType === 'techreport' ? "Technical report" : '';
...
export function computeBibliographyHealth(references: ThesisReference[]): BibliographyHealth {
  ...
  for (const ref of references) {
    ...
    const isMasterThesisHealth = refType === 'thesis' && ref.degree && ref.degree.toLowerCase() !== 'phd';
    const bibType: BibEntryType =
      refType === 'thesis' ? (isMasterThesisHealth ? 'mastersthesis' : 'phdthesis') :
