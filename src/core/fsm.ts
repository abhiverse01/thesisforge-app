...
export interface WizardState {
  ...
  /** Internal step health data — NOT part of exported thesis data. */
  _stepHealth?: StepHealth;
}
...
const defaultGuards: TransitionGuards = {
  templateSelected: (data) => !!data.templateId && typeof data.templateId === 'string' && data.templateId.trim() !== '',
  metadataValid: (data) => {
    const meta = data.metadata as Record<string, unknown> | undefined;
...
export function transition(
  ...
  switch (event) {
    ...
    case 'JUMP': {
      if (jumpTarget === undefined || jumpTarget < 1 || jumpTarget >= STATE_ORDER.length) return state;
      const targetStep = STATE_ORDER[jumpTarget];
      if (jumpTarget <= currentIdx) return { ...state, step: targetStep, stepIndex: jumpTarget, errors: {} };
      const jumpValidation = validateJump(state.step, targetStep, state.data, guards);
      if (!jumpValidation.canJump) {
        const errors: Record<string, string> = {};
        for (const incomplete of jumpValidation.incompleteSteps) {
          errors[incomplete.step] = incomplete.issues.join('; ');
        }
        return { ...state, errors };
      }
      return { ...state, step: targetStep, stepIndex: jumpTarget, errors: {} };
    }
    ...
    case 'STEP_HEALTH_CHECK': {
      // Store health data on the state object itself — NOT in state.data.
      // Embedding in state.data would pollute thesis data on save/reload.
  ...
  for (let i = currentIdx + 1; i < targetIdx; i++) {
    ...
    switch (stepName) {
      ...
      case 'METADATA':
        ...
        break;
      case 'CHAPTERS':
        if (!guards.chaptersValid(data)) return false;
        break;
    }
  }
