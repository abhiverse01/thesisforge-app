...
const ENGINE_CONTRACT: ContractCheck[] = [
  ...
  { id: 'A07', severity: 'error',
    check: (tex: string) => {
      // Only check inside float environments (figure, table).
      // \label before \caption inside a float means the reference number
      // will be wrong because \caption triggers the counter increment.
      const floats = [...tex.matchAll(/\\begin\{(figure|table)\}([\s\S]*?)\\end\{\1\}/g)];
      for (const match of floats) {
        const body = match[2];
        if (/\\label\{[^}]*\}[\s\S]*?\\caption/.test(body)) return false;
      }
      return true;
    },
