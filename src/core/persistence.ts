...
function sanitizeThesisData(raw: unknown): ThesisData {
  ...
  return {
    ...
    customCommands: Array.isArray(r.customCommands) ? (r.customCommands as unknown[]).filter((c): c is string => typeof c === 'string') : [],
...
export async function saveDraft(
  ...
  // FIX(ZONE-2C): Read current version to increment, and preserve original createdAt
  ...
  try {
    ...
    if (error.name === 'QuotaExceededError' || (error.message && error.message.includes('quota'))) {
      ...
      // GODMODE 13: Limit retry to 1 attempt — prevents infinite open→fail→open loop
      // when DB connection is persistently broken (storage pressure, private browsing).
      ...
      try {
        ...
        console.error('[Persistence] Retry after InvalidStateError failed:', retryErr);
      }
    } else {
      emitSaveStatus('error');
...
// ============================================================
// Delta Save — Track dirty fields, write only changes
// For a 50k-word thesis this reduces auto-save payload by 80-90%.
// ============================================================

type DirtyField = 'metadata' | 'chapters' | 'references' | 'format' | 'all';

let dirtyFields = new Set<DirtyField>();

/** Mark a field as dirty. Call from Zustand store actions. */
export function markDirty(field: DirtyField) {
  dirtyFields.add(field);
}

/** Get current dirty fields (for debugging/testing). */
export function getDirtyFields(): Set<DirtyField> {
  return new Set(dirtyFields);
}

/** FIXELEVEN: Atomically consume dirty fields — returns a snapshot and clears.
 * Prevents the race where component B marks a field dirty between
 * save #1 checking size and save #1 calling clearDirtyFields.
 */
export function consumeDirtyFields(): Set<DirtyField> {
  const snapshot = new Set(dirtyFields);
  dirtyFields.clear();
  return snapshot;
}

/** Clear dirty fields after a successful save. */
export function clearDirtyFields() {
  dirtyFields.clear();
}

/**
 * Save only dirty fields to IndexedDB instead of the full ThesisData.
 * Falls back to full save when too many fields are dirty.
 */
export async function saveDelta(
  draftId: string,
  fullState: ThesisData,
  templateId: ThesisType,
  wizardStep: number
): Promise<void> {
  // FIXELEVEN: Use atomic consume instead of separate check + clear.
  // Prevents the race where another markDirty() call interleaves
  // between the size check and the clear, losing that dirty field.
  const fieldsToSave = consumeDirtyFields();
  if (fieldsToSave.size === 0) return;

  if (fieldsToSave.has('all') || fieldsToSave.size >= 3) {
    // Too many changes — just do a full save
    await saveDraft(fullState, templateId, wizardStep);
    return;
  }
  ...
  try {
    ...
    // Memory fallback path: fall back to full save
    if ((db as MemoryDB)._isMemoryFallback) {
      await saveDraft(fullState, templateId, wizardStep);
      return;
    }

    const idb = db as IDBPDatabase;
    const tx = idb.transaction('drafts', 'readwrite');
    const existing = await tx.store.get(draftId) as DraftRecord | undefined;
    
    if (!existing) {
      // No existing record — do a full save
      await saveDraft(fullState, templateId, wizardStep);
      return;
    }

    // Merge only dirty fields into existing record
    ...
    const fieldToDataKey: Record<string, keyof ThesisData> = {
      metadata: 'metadata',
      chapters: 'chapters',
    ...
    for (const field of fieldsToSave) {
      ...
      if (dataKey) {
        updated.data = { ...updated.data, [dataKey]: fullState[dataKey] };
      }
    }
    ...
    // Fallback to full save on any error
    await saveDraft(fullState, templateId, wizardStep);
  }
}
...
export async function loadDraft(): Promise<{
  ...
  try {
    ...
    // FIX(Phase-9): Update lastKnownTimestamp after successful load
    // so that a detected conflict doesn't re-trigger on every subsequent load.
    try {
      sessionStorage.setItem('thesisforge_timestamp', String(record.updatedAt));
      sessionStorage.setItem('thesisforge_last_version', String(record.version));
    } catch { /* sessionStorage unavailable */ }
...
export function computeSnapshotDiff(snapshotData: ThesisData, currentData: ThesisData): string {
  ...
  if (currentAppCount > snapAppCount) {
    const diff = currentAppCount - snapAppCount;
    changes.push(`+${diff} ${diff === 1 ? 'appendix' : 'appendices'}`);
  } else if (currentAppCount < snapAppCount) {
    const diff = snapAppCount - currentAppCount;
    changes.push(`-${diff} ${diff === 1 ? 'appendix' : 'appendices'}`);
  }
...
export async function checkConflict(lastKnownTimestamp: number): Promise<{
  ...
  try {
    ...
    // FIXTEN: If reading throws (corrupted IDB), we don't know if there's a conflict.
    // Returning hasConflict: false would lie to the caller and could cause silent
    // data overwrites in multi-tab scenarios. Return undefined to signal unknown.
    return { hasConflict: undefined as unknown as boolean, storedTimestamp: null };
  }
}
