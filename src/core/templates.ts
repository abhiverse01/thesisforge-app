...
export const TEMPLATE_SCHEMAS: Record<string, TemplateSchema> = {
  bachelor: {
    ...
    bodyStructure: ['introduction', 'literature review', 'methodology', 'results', 'discussion', 'conclusion', 'references'],
    ...
    validators: {
      ...
      optionalFields: ['subtitle', 'department', 'faculty', 'coSupervisor', 'location', 'dedication', 'acknowledgment', 'orcid'],
    },
    compilationRecipe: {
      compiler: 'pdflatex',
      passes: 4,
  ...
  master: {
    ...
    validators: {
      ...
      optionalFields: ['subtitle', 'faculty', 'coSupervisor', 'location', 'dedication', 'acknowledgment', 'orcid'],
    },
    compilationRecipe: {
      compiler: 'pdflatex',
      passes: 4,
  ...
  phd: {
    ...
    bodyStructure: ['introduction', 'literature review', 'methodology', 'results I', 'results II', 'discussion', 'conclusion', 'future work'],
    backMatter: ['bibliography', 'appendix'],
    citationStyle: 'plainnat',
    ...
    validators: {
      ...
      optionalFields: ['subtitle', 'coSupervisor', 'location', 'dedication', 'acknowledgment', 'orcid'],
    },
    compilationRecipe: {
      compiler: 'pdflatex',
      passes: 4,
  ...
  report: {
    ...
    bodyStructure: ['introduction', 'methods', 'results', 'discussion', 'conclusion', 'references'],
    backMatter: ['bibliography'],
    citationStyle: 'plainnat',
    ...
    validators: {
      ...
      optionalFields: ['subtitle', 'university', 'department', 'supervisor', 'location', 'reportNumber'],
    },
    compilationRecipe: {
      compiler: 'pdflatex',
      passes: 2,
  ...
  conference: {
    ...
    bodyStructure: ['introduction', 'related work', 'methodology', 'experiments', 'results', 'conclusion', 'references'],
    backMatter: ['bibliography'],
    ...
    validators: {
      requiredFields: ['title', 'author'],
      optionalFields: ['university', 'department', 'orcid'],
    },
    compilationRecipe: {
      compiler: 'pdflatex',
      passes: 2,
...
// ============================================================
// Document Class Labels — for UI display
// ============================================================
...
/**
 * Get the human-readable document class name for a template.
 */
export function getDocumentClassLabel(templateId: string): string {
  const schema = TEMPLATE_SCHEMAS[templateId];
  if (!schema) return 'Unknown';
  return schema.documentClass;
}
