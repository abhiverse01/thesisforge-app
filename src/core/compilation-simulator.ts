...
// ============================================================
// Simulation Depth — Gated Pass Execution
// quick: Passes 1+2 only (live preview, <20ms)
// full:  All passes (export time)
// ============================================================

export type SimulationDepth = 'quick' | 'full';
...
/**
 * Quick simulation — Passes 1 + 2 only (brace balance + package resolver).
 * Used for live preview panel — fast enough for debounced 500ms calls.
 * Returns in <20ms on large thesis.
 */
export function simulateQuick(
  texContent: string,
  bibContent?: string
): SimulationResult {
  const pass1 = runTokenizer(texContent);
  ...
  const pass2 = runPackageResolver(texContent);

  return {
    // FIX: passed should reflect whether there are actual errors
    passed: [...pass1.errors, ...pass2.errors].length === 0,
    errors: [...pass1.errors, ...pass2.errors],
    warnings: pass2.warnings,
    info: pass2.info,
    estimatedPageCount: 0,
    compilationRecipe: [],
    packageSummary: pass2.packageSummary,
    labelSummary: { defined: 0, referenced: 0, unresolved: [] },
    citationSummary: { totalCitations: 0, definedKeys: 0, unresolvedKeys: [] },
  };
}
...
function runTokenizer(texContent: string): {
  ...
  while (i < len) {
    ...
    // FIX: Skip if preceded by \ (escaped \% is a literal percent, not a comment)
    if (ch === '%' && !(i > 0 && texContent[i - 1] === '\\')) {
      const start = i;
          ...
          if (topEnv === envName) {
            ...
            if (topEnv !== undefined) {
              errors.push({
                code: 'SIM-E007',
                severity: 'error',
                message: `Mismatched environment: \\begin{${topEnv}} closed by \\end{${envName}}. Expected \\end{${topEnv}}.`,
                location: `line ${line}`,
                hint: `Change \\end{${envName}} to \\end{${topEnv}}.`,
              });
            }
  ...
  // Odd number of $ means unclosed inline math
  const dollarPositions = tokens.filter(t => t.type === 'math-inline');
  if (dollarPositions.length % 2 !== 0) {
    errors.push({
      code: 'SIM-E006',
      severity: 'error',
      message: 'Unclosed inline math: odd number of $ signs. Each $ must have a matching closing $.',
      location: `line ${dollarPositions[dollarPositions.length - 1].line}`,
      hint: 'Add a closing $ or escape the $ with \\$.',
    });
  }
  ...
  // Check bracket depth
  // FIX: Skip brackets that are package options \usepackage[...]{...}
  // These are valid optional arguments and should not affect bracket balance.
  // Track which bracket-open tokens are flagged as package options so their
  // matching bracket-close can also be skipped.
  const pkgOptionBrackets = new Set<number>();
  let bracketDepth = 0;
  for (let ti = 0; ti < tokens.length; ti++) {
    const t = tokens[ti];
    if (t.type === 'bracket-open') {
      // Check if this bracket is a package option: look back for \usepackage command
      let isPkgOption = false;
      for (let j = ti - 1; j >= 0 && j >= ti - 3; j--) {
        if (tokens[j].type === 'command' && tokens[j].value === 'usepackage') {
          isPkgOption = true;
          break;
        }
        if (tokens[j].type !== 'text' || tokens[j].value.trim() !== '') break;
      }
      if (isPkgOption) {
        pkgOptionBrackets.add(ti);
      } else {
        bracketDepth++;
      }
    } else if (t.type === 'bracket-close') {
      // Check if this closes a package option bracket by finding the
      // matching bracket-open and checking if it was flagged.
      let isPkgOptionClose = false;
      let depth = 0;
      for (let j = ti - 1; j >= 0; j--) {
        if (tokens[j].type === 'bracket-close') depth++;
        if (tokens[j].type === 'bracket-open') {
          if (depth === 0) {
            // Found matching open bracket
            if (pkgOptionBrackets.has(j)) {
              isPkgOptionClose = true;
            }
            break;
          }
          depth--;
      ...
      if (!isPkgOptionClose) bracketDepth--;
    }
  }
  if (bracketDepth !== 0) {
    errors.push({
...
function runCommandValidator(
  ...
  const PACKAGE_COMMANDS: Record<string, string[]> = {
    ...
    // FIX: Key must match the package name as stored in loadedPackages (with hyphen, not underscore)
    'unicode-math': ['setmathfont', 'symup', 'symit', 'symbf', 'symcal', 'symscr', 'symfrak', 'symbb'],
...
const COMMON_MISTAKES: Array<{
  pattern?: RegExp;
  check?: (texContent: string) => boolean;
  message: string;
  fix: string;
  ...
  {
    check: (texContent: string) => {
      // Balanced-brace extractor: finds \footnote{ and counts braces to find matching }
      const footnoteRegex = /\\footnote\{/g;
      let m: RegExpExecArray | null;
      while ((m = footnoteRegex.exec(texContent)) !== null) {
        const start = m.index + m[0].length;
        let depth = 1;
        let i = start;
        while (i < texContent.length && depth > 0) {
          if (texContent[i] === '{') depth++;
          else if (texContent[i] === '}') depth--;
          i++;
        }
        if (depth === 0 && (i - start) > 200) {
          return true;
        }
      }
      return false;
  ...
  {
    check: (texContent: string) => {
      // Strip tabular/align/array/matrix environments before checking for \\
      const stripped = texContent.replace(
        /\\begin\{(tabularx?|align\*?|alignat\*?|gather\*?|multline\*?|array|matrix|bmatrix|pmatrix|vmatrix|cases)\}[\s\S]*?\\end\{\1\}/gi,
        ''
      );
  ...
  {
    pattern: /\\label\{[^}]+\}\s*\\caption\{[^}]+\}/,
    message: '\\label must come after \\caption, not before. Move \\label after \\caption{...} or use both in the correct order.',
    fix: 'Move \\label after \\caption{...}',
    severity: 'warning',
  },
  {
  ...
  {
    // FIX: Original regex with [\s\S]*? was non-greedy and could match to EOF.
    // Improved: check that the first non-empty line after \begin{tabular} isn't \toprule.
    pattern: /\\begin\{tabular[\w*]*\}[ \t]*\n[ \t]*(?!\\toprule)[^\n\\]/,
...
export function runPass3Extended(
  ...
  return COMMON_MISTAKES
    .filter(rule => (rule.pattern ? rule.pattern.test(texContent) : rule.check!(texContent)))
    .map((rule, index) => ({
...
function runBibtexResolver(
  ...
  // FIX: Unresolved citations are warnings, not errors — real LaTeX compiles
  // with undefined citation warnings (resolved on subsequent bibtex runs).
...
export function simulateCompilation(
  ...
  // ── Phase 3 Extended: Common student mistakes ──────
  const extendedWarnings = runPass3Extended(texContent);
  for (const ew of extendedWarnings) {
    if (ew.severity === 'error') {
      allErrors.push({
        code: ew.code,
        severity: 'error',
        message: ew.message,
