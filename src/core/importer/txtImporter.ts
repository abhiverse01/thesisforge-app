...
function extractPlainTextChapters(lines: string[]): ExtractedChapter[] {
  ...
  // Pattern 4: Lines followed by many dashes or equals signs (underlined headings)
  const underlinedRe = /^([A-Za-z][A-Za-z\s&:–—]{5,80})\n\s*[-=]{3,}$/m;
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (i > 2 && allCapsRe.test(line)) {
      ...
      if (skipTitles.test(title)) continue;
      // Only add if the line is reasonably long and the next non-empty line has content
  ...
  for (let i = 0; i < headingPositions.length; i++) {
    ...
    for (let j = start; j < end; j++) {
      ...
      if (!subMatch) {
        const numMatch = /^\s*(\d+\.\d+)\s+([A-Za-z][A-Za-z\s&:]{5,60})$/.exec(line);
        if (numMatch) {
