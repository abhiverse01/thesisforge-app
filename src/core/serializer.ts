...
\\end{table}`;
    ...
    case 'MultilineMath': {
      ...
      // FIX: Label must come AFTER content for correct equation numbering.
      // Placing \\label before the formula gives wrong equation numbers.
      result += pad + `  ${content}\n`;
      if (mm.label) {
    ...
    case 'HyperLink': {
      ...
      // FIX: URL must NOT be escaped — \\href{}{} treats its first argument as verbatim.
      // Only the display text needs LaTeX escaping.
...
function isMajorSection(node: ASTNode): boolean {
  ...
  if (node.type === 'Environment') {
    ...
    return ['document', 'titlepage', 'thebibliography', 'abstract'].includes(envName);
  }
  if (node.type === 'Command') {
    const cmdName = (node as CommandNode).name;
    // FIX: Removed 'begin' — it matches ALL environments (itemize, figure, etc.)
    // causing excess blank lines. Environment node type is handled separately above.
...
// ============================================================
// Streaming Serializer — Generator-based output
// Yields chunks of LaTeX string instead of building one huge string.
// Use serializeToString() for synchronous collection,
// or serializeAsync() for progressive output with yielding to main thread.
// ============================================================

/**
 * Generator-based serializer that yields string chunks.
 * Falls back to synchronous serialize() for complex node types.
 */
export function* serializeStream(node: ASTNode): Generator<string> {
  switch (node.type) {
    case 'Document': {
      for (const child of (node as DocumentNode).children) {
        yield* serializeStream(child);
        yield '\n';
      }
      break;
    }
    ...
    case 'Command': {
      // Fall back to sync serializer for commands (handles all arg types)
      yield serialize(node);
      break;
    }

    default:
      // Fall back to sync serializer for all other node types
      yield serialize(node);
  }
}
 ...
 * Equivalent to serializeDocument() but uses the streaming path.
 */
export function serializeToString(node: ASTNode): string {
  return [...serializeStream(node)].join('');
}

/**
 * Async streaming serializer — yields to the main thread periodically.
 * Use when the document is large and you want to avoid blocking.
 */
export async function serializeAsync(
  ...
  for (const chunk of serializeStream(node)) {
    chunks.push(chunk);
    yielded++;
    if (yielded % 200 === 0) {
      onProgress(Math.min(95, yielded / 10));
