...
export type CitationStyle = 'ieee' | 'apa' | 'vancouver' | 'chicago' | 'harvard' | 'plainnat';
...
export interface ThesisReference {
  ...
  eprintType?: string;
  crossRef?: string;
  /** Degree level for thesis entries (e.g. "phd", "master", "MSc", "Ph.D.") */
  degree?: string;
}
...
// Abstract word limits per thesis type — enforced at export time as soft warning
...
// Minimum references required per thesis type at export — guides completion scoring
export const MIN_REFERENCES: Record<ThesisType, number> = {
  bachelor: 5,
  master: 10,
  phd: 20,
  report: 3,
...
// Note shown when user skips references step — communicates deferred requirement
export const REFERENCES_SKIPPABLE_NOTE = 'References can be added later. Minimum required at export depends on thesis type.';
...
export function createDefaultThesisData(type: ThesisType): ThesisData {
  ...
  if (!template) {
    throw new Error(`Unknown thesis type: "${type}". Valid types are: ${THESIS_TEMPLATES.map(t => t.type).join(', ')}`);
  }
  return {
    type,
    metadata: {
