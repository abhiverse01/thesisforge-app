...
import type { StructureAnalysis, StructureIssue, StructuralWarning } from './types';
...
/**
 * Recommended total word count ranges per thesis type.
 */
const WORD_COUNT_NORMS: Record<string, { min: number; max: number; label: string }> = {
  bachelor: { min: 8000, max: 15000, label: "Bachelor's Thesis" },
  master: { min: 15000, max: 40000, label: "Master's Thesis" },
  phd: { min: 50000, max: 100000, label: 'PhD Thesis' },
  report: { min: 3000, max: 8000, label: 'Research Report' },
  conference: { min: 4000, max: 8000, label: 'Conference Paper' },
};

/** Maximum percentage a single chapter should occupy (40%). */
const CHAPTER_MAX_PCT = 0.40;

/** Minimum percentage a single chapter should occupy (5%). */
const CHAPTER_MIN_PCT = 0.05;
  ...
  for (const ch of wordCounts) {
    ...
    if (actual < norm.min) {
      issues.push({
        ...
        severity: 'medium',
        words: ch.words,
        suggestion: generateBalanceSuggestion({ chapterId: ch.id, chapterTitle: ch.title, actualPct: Math.round(actual * 100), idealPct: Math.round(((norm.min + norm.max) / 2) * 100), direction: 'under', severity: 'medium', words: ch.words }),
      });
      ...
      issues.push({
        ...
        severity: 'medium',
        words: ch.words,
        suggestion: generateBalanceSuggestion({ chapterId: ch.id, chapterTitle: ch.title, actualPct: Math.round(actual * 100), idealPct: Math.round(((norm.min + norm.max) / 2) * 100), direction: 'over', severity: 'medium', words: ch.words }),
      });
    }
  }
...
export function analyzeStructure(
  ...
  for (const ch of wordCounts) {
    ...
    if (Math.abs(delta) > norm.tolerance) {
      issues.push({
        ...
        words: ch.words,
        suggestion: generateBalanceSuggestion({ chapterId: ch.id, chapterTitle: ch.title, actualPct: Math.round(actual * 100), idealPct: Math.round(norm.ideal * 100), direction: delta > 0 ? 'over' : 'under', severity: Math.abs(delta) > norm.tolerance * 2 ? 'high' : 'medium', words: ch.words }),
      });
    }
  }
  ...
  // Check word count against academic norms
  const wordCountNorm = WORD_COUNT_NORMS[templateId];
  if (wordCountNorm) {
    if (totalWords < wordCountNorm.min) {
      issues.push({
        chapterId: 'total',
        chapterTitle: 'Total Word Count',
        actualPct: 0,
        idealPct: 100,
        direction: 'under',
        severity: 'medium',
        words: totalWords,
        suggestion: `Total word count is ${totalWords.toLocaleString()}, below the recommended minimum of ${wordCountNorm.min.toLocaleString()} for a ${wordCountNorm.label}. Consider expanding your chapters.`,
      });
    } else if (totalWords > wordCountNorm.max) {
      issues.push({
        chapterId: 'total',
        chapterTitle: 'Total Word Count',
        actualPct: 0,
        ...
        severity: 'medium',
        words: totalWords,
        suggestion: `Total word count is ${totalWords.toLocaleString()}, exceeding the recommended maximum of ${wordCountNorm.max.toLocaleString()} for a ${wordCountNorm.label}. Consider tightening your writing or moving supplementary content to appendices.`,
      });
    }
  }

  // Check chapter balance: flag if any single chapter is >40% or <5% of total
  for (const ch of wordCounts) {
    const pct = ch.words / totalWords;
    if (pct > CHAPTER_MAX_PCT) {
      const alreadyFlagged = issues.some(i => i.chapterId === ch.id);
      if (!alreadyFlagged) {
        issues.push({
          chapterId: ch.id,
          chapterTitle: ch.title,
          actualPct: Math.round(pct * 100),
          idealPct: 100 / wordCounts.length,
          direction: 'over',
          severity: 'high',
          words: ch.words,
          suggestion: `"${ch.title}" occupies ${Math.round(pct * 100)}% of your thesis (>${Math.round(CHAPTER_MAX_PCT * 100)}%). This is disproportionately large. Consider splitting into multiple chapters or moving content to appendices.`,
        });
      }
    } else if (pct < CHAPTER_MIN_PCT && ch.words > 0) {
      const alreadyFlagged = issues.some(i => i.chapterId === ch.id);
      if (!alreadyFlagged) {
        issues.push({
          chapterId: ch.id,
          ...
          severity: 'medium',
          words: ch.words,
          suggestion: `"${ch.title}" occupies only ${Math.round(pct * 100)}% of your thesis (<${Math.round(CHAPTER_MIN_PCT * 100)}%). This chapter may be too short. Consider merging with an adjacent chapter or expanding the content.`,
        });
      }
    }
  }
  ...
  // Detect orphan sections and title-only chapters
  const structuralWarnings = detectStructuralWarnings(chapters);
  ...
  const suggestions: string[] = [
    ...
    ...generateStructuralWarningSuggestions(structuralWarnings),
  ];
...
/**
 * Detect structural warnings: orphan sections and title-only chapters.
 *
 * Orphan sections: subsections defined in the subsections array but with
 * empty or missing content (fewer than 5 words).
 *
 * Title-only chapters: chapters where the body has fewer than 50 words,
 * suggesting the chapter is likely just a placeholder.
 */
function detectStructuralWarnings(chapters: ThesisChapter[]): StructuralWarning[] {
  const warnings: StructuralWarning[] = [];
  const ORPHAN_THRESHOLD = 5;
  const TITLE_ONLY_THRESHOLD = 50;

  for (const ch of chapters) {
    // Detect title-only chapters (body < 50 words)
    const bodyWordCount = countWords(ch.content);
    if (bodyWordCount < TITLE_ONLY_THRESHOLD && bodyWordCount >= 0) {
      warnings.push({
        type: 'title-only-chapter',
        chapterId: ch.id,
        chapterTitle: ch.title,
        wordCount: bodyWordCount,
        message: `"${ch.title}" appears to be a placeholder chapter (${bodyWordCount} word${bodyWordCount === 1 ? '' : 's'} in body). Add substantive content or remove this chapter.`,
      });
    }

    // Detect orphan sections (subsections with empty/missing content)
    if (ch.subSections && ch.subSections.length > 0) {
      for (const ss of ch.subSections) {
        const ssWordCount = countWords(ss.content);
        if (ssWordCount < ORPHAN_THRESHOLD) {
          warnings.push({
            type: 'orphan-section',
            chapterId: ch.id,
            chapterTitle: ch.title,
            sectionTitle: ss.title,
            wordCount: ssWordCount,
            message: `Subsection "${ss.title}" in "${ch.title}" has no content (${ssWordCount} word${ssWordCount === 1 ? '' : 's'}). Fill it in or remove it from the outline.`,
          });
        }
      }
    }
...
/**
 * Generate suggestions from structural warnings.
 */
function generateStructuralWarningSuggestions(warnings: StructuralWarning[]): string[] {
  return warnings.map((w) => {
    if (w.type === 'title-only-chapter') {
      return `[Placeholder] ${w.message}`;
    }
    return `[Orphan Section] ${w.message}`;
  });
}
