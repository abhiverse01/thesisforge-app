...
export function buildCitationGraph(
  ...
  // Issue #14 fix: Snapshot data at export time to avoid stale closure refs
  // if the caller mutates chapters/references after buildCitationGraph returns.
  function exportAsDot(): string {
    ...
    // Chapter nodes — snapshot titles to avoid mutation via closure
    lines.push('  // Chapters');
    for (const ch of chapters) {
      const chId = ch.id || 'unknown';
      const chTitle = String(ch.title || chId).replace(/"/g, '\\"');
    ...
    // Edges: chapters → references — use snapshot copies of the Maps
    lines.push('  // Citation edges');
    const chapterSnap = new Map(chapterCitations);
    for (const [chapterId, keys] of chapterSnap) {
      const keySnap = new Set(keys);
      for (const key of keySnap) {
