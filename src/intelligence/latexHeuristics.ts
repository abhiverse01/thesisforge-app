...
const HEURISTIC_RULES: HeuristicRule[] = [
  ...
  // === 3 NEW STRUCTURAL RULES ===

  // 16. Excessive footnotes (>5 per chapter)
  // Detected via post-processing (see checkFootnoteOveruse below)
  {
    id: 'footnote-overuse',
    pattern: /\\footnote\{/g,
    severity: 'info',
    message: (match) =>
      'Multiple footnotes detected. If there are more than 5 footnotes in a chapter, consider using endnotes or integrating content into the main text.',
    autofix: null,
  },

  // 17. \label{} without corresponding \ref{}
  // Detected via post-processing (see checkUnreferencedLabels below)
  {
    id: 'unreferenced-label',
    pattern: /\\label\{([^}]+)\}/g,
    severity: 'warning',
    message: (match) =>
      `Label "${match[1]}" is defined but never referenced. Use \\ref{${match[1]}} or \\eqref{${match[1]}} somewhere in the document, or remove the unused label.`,
    autofix: null,
  },

  // 18. Empty citation keys: \cite{} with empty key
  {
    id: 'empty-citation-key',
    pattern: /\\cite(?:p|t|author|year|alp|num)?\{\s*\}/g,
    severity: 'error',
    message: () =>
      'Empty citation key detected (\\cite{} with no key). This will cause a LaTeX compilation error. Add the correct citation key or remove the empty cite command.',
    autofix: null,
  },
];
...
function stripCodeBlocks(text: string): string {
  return stripLatexEnvironments(text);
}

/**
 * Strip all environments and math mode from text before pattern matching.
 * Prevents false positives from smart quotes inside math mode,
 * \begin{verbatim}, lstlisting, minted, equation, align, etc.
 *
 * WHY: Smart quotes (\u201C etc.) inside $...$ or $$...$$ are NOT LaTeX
 * anti-patterns — they're math content that should be preserved.
 */
export function stripLatexEnvironments(text: string): string {
  return text
    // Verbatim-like environments (must be first — they can contain anything)
    ...
    // Math environments
    .replace(/\\begin\{(?:equation|align|math|gather|multline|eqnarray\*?)\}[\s\S]*?\\end\{(?:equation|align|math|gather|multline|eqnarray\*?)\}/g, '')
    // Display math $$...$$
    .replace(/\$\$[\s\S]*?\$\$/g, '')
    // Display math \[...\]
    .replace(/\\\[[\s\S]*?\\\]/g, '')
...
export function runHeuristics(text: string): HeuristicFinding[] {
  ...
  // Post-processing: check for excessive footnotes (>5 per section)
  checkFootnoteOveruse(cleanedText, findings);

  // Post-processing: check for \label{} without corresponding \ref{}
  checkUnreferencedLabels(cleanedText, findings);
...
/**
 * Post-process: detect excessive footnotes (>5 per section).
 */
function checkFootnoteOveruse(text: string, findings: HeuristicFinding[]): void {
  const FOOTNOTE_THRESHOLD = 5;

  // Split into sections (by \section, \chapter, etc.)
  const sections = text.split(/\\(?:section|chapter|paragraph|subsection)\{/);
  const sectionPattern = /\\footnote\{/g;

  let currentOffset = 0;
  for (const section of sections) {
    const footnoteMatches = section.match(sectionPattern);
    if (footnoteMatches && footnoteMatches.length > FOOTNOTE_THRESHOLD) {
      const sectionStart = text.indexOf(section, currentOffset);
      if (sectionStart >= 0) {
        findings.push({
          ruleId: 'footnote-overuse-detail',
          severity: 'info',
          offset: sectionStart,
          length: section.length,
          original: section.slice(0, 60) + '...',
          message: `Section contains ${footnoteMatches.length} \\footnote commands (threshold: ${FOOTNOTE_THRESHOLD}). Consider using endnotes or integrating content into the main text.`,
          fix: null,
        });
      }
    }
    currentOffset += section.length + 10; // approximate offset for the split command
  }
}

/**
 * Post-process: detect \label{} definitions without corresponding \ref{} or \eqref{}.
 */
function checkUnreferencedLabels(text: string, findings: HeuristicFinding[]): void {
  // Find all \label{key} definitions
  const labelPattern = /\\label\{([^}]+)\}/g;
  const labels: Array<{ key: string; index: number }> = [];
  let m: RegExpExecArray | null;
  while ((m = labelPattern.exec(text)) !== null) {
    labels.push({ key: m[1], index: m.index });
  }

  for (const { key, index } of labels) {
    // Check if this label is referenced anywhere with \ref, \eqref, \autoref, \cref, \pageref
    const escapedKey = key.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    const refPattern = new RegExp(`\\\\(?:ref|eqref|autoref|cref|pageref|nameref)\\{${escapedKey}\\}`, 'i');
    if (!refPattern.test(text)) {
      findings.push({
        ruleId: 'unreferenced-label-detail',
        severity: 'warning',
        offset: index,
        length: key.length + 8, // \label{key}
        original: `\\label{${key}}`,
        message: `Label "${key}" is defined but never referenced. Use \\ref{${key}} or \\eqref{${key}} somewhere in the document, or remove the unused label.`,
        fix: null,
      });
    }
  }
