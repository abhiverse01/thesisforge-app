...
import type { CompletenessResult, CompletenessIssue, RubricItem, SubScores, RadarDataPoint } from './types';
...
function evaluateField(field: string, data: Record<string, unknown>): boolean {
  ...
  if (field === 'chapters.allHaveContent') {
    ...
    return chapters.every((ch) => countWords(ch.content) >= 5);
  }
...
/**
 * Abstract word count threshold above which a slight penalty is applied.
 * Most universities penalize overlong abstracts (typically > 350 words).
 */
const ABSTRACT_OVERLONG_THRESHOLD = 350;

/**
 * Minimum word count for a chapter body to be considered substantive.
 * Chapters below this threshold that aren't optional generate a warning.
 */
const MIN_CHAPTER_BODY_WORDS = 100;

/**
 * Chapters whose titles match these patterns are considered optional
 * (appendices, references, acknowledgments, etc.) and exempt from the
 * minimum word count warning.
 */
const OPTIONAL_CHAPTER_PATTERNS = [
  /appendix|annex/i,
  ...
  /list of (figures|tables|symbols)/i,
  /curriculum vitae|cv|resume/i,
];
 ...
 * - Abstract overlength penalty (> 350 words)
 * - Short chapter body warnings (< 100 words for non-optional chapters)
 * - readyForExport flag: true when score >= 75 AND no critical issues
 *
 * Edge cases:
...
export function scoreCompleteness(
  ...
  // --- Compute additional issues ---
  const issues: CompletenessIssue[] = [];

  if (wizardData) {
    // Check for overlong abstract (> 350 words)
    const abstractWordCount = countWords(wizardData.abstract);
    if (abstractWordCount > ABSTRACT_OVERLONG_THRESHOLD) {
      issues.push({
        severity: 'warning',
        message: `Abstract is ${abstractWordCount} words (over ${ABSTRACT_OVERLONG_THRESHOLD}). Most universities penalize overlong abstracts. Consider trimming to stay within the recommended limit.`,
      });
    }

    // Check for insufficient references for master's/PhD theses (< 10 refs)
    const refCount = wizardData.references.length;
    if ((templateId === 'master' || templateId === 'phd') && refCount > 0 && refCount < 10) {
      issues.push({
        severity: 'warning',
        message: `Only ${refCount} reference(s) found. A ${templateId === 'phd' ? 'PhD' : "master's"} thesis typically requires at least 10 references. Consider adding more relevant sources.`,
      });
    }

    // Check for short chapter bodies (< 100 words for non-optional chapters)
    for (const ch of wizardData.chapters) {
      const bodyWordCount = countWords(ch.content);
      if (bodyWordCount > 0 && bodyWordCount < MIN_CHAPTER_BODY_WORDS) {
        const isOptional = OPTIONAL_CHAPTER_PATTERNS.some((pat) => pat.test(ch.title));
        if (!isOptional) {
          issues.push({
            severity: bodyWordCount < 20 ? 'critical' : 'warning',
            message: `Chapter "${ch.title}" has only ${bodyWordCount} word${bodyWordCount === 1 ? '' : 's'} in its body. This is below the ${MIN_CHAPTER_BODY_WORDS}-word minimum for substantive chapters.`,
            chapterId: ch.id,
  ...
  // Determine readyForExport: score >= 75 AND no critical issues
  const hasCriticalIssues = issues.some((i) => i.severity === 'critical');
  const finalScore = Math.round(ratio * 100);
  const readyForExport = finalScore >= 75 && !hasCriticalIssues;
