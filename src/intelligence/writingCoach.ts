...
export function checkS01_IntroGap(
  thesis: ThesisData,
  out: CoachSuggestion[],
  dismissed: Set<string>,
  _refMap?: Map<string, any>   // new optional param
): void {
...
export function checkS02_MethodJustification(
  thesis: ThesisData,
  out: CoachSuggestion[],
  dismissed: Set<string>,
  _refMap?: Map<string, any>   // new optional param
...
export function checkS03_ResultsInterpretation(
  thesis: ThesisData,
  out: CoachSuggestion[],
  dismissed: Set<string>,
  _refMap?: Map<string, any>   // new optional param
...
export function checkS04_DiscussionRQReference(
  thesis: ThesisData,
  out: CoachSuggestion[],
  dismissed: Set<string>,
  _refMap?: Map<string, any>   // new optional param
...
export function checkS05_ConclusionNewRefs(
  thesis: ThesisData,
  out: CoachSuggestion[],
  dismissed: Set<string>,
  _refMap?: Map<string, any>   // new optional param
...
export function checkA01_TransitionalClosing(
  thesis: ThesisData,
  out: CoachSuggestion[],
  dismissed: Set<string>,
  _refMap?: Map<string, any>   // new optional param
...
export function checkA02_ClaimDensity(
  thesis: ThesisData,
  out: CoachSuggestion[],
  dismissed: Set<string>,
  _refMap?: Map<string, any>   // new optional param
...
export function checkA03_StructuralMonotony(
  thesis: ThesisData,
  out: CoachSuggestion[],
  dismissed: Set<string>,
  _refMap?: Map<string, any>   // new optional param
...
export function checkA04_LiteratureSynthesis(
  thesis: ThesisData,
  out: CoachSuggestion[],
  dismissed: Set<string>,
  _refMap?: Map<string, any>   // new optional param
...
export function checkA05_ContributionEcho(
  thesis: ThesisData,
  out: CoachSuggestion[],
  dismissed: Set<string>,
  _refMap?: Map<string, any>   // new optional param
...
export function checkC01_FirstAuthorOverReliance(
  thesis: ThesisData,
  out: CoachSuggestion[],
  dismissed: Set<string>,
...
export function checkC02_NoRecentCitations(
  thesis: ThesisData,
  out: CoachSuggestion[],
  dismissed: Set<string>,
  refMap: Map<string, typeof thesis.references[number]>
...
export function checkC03_AuthorProminentOveruse(
  thesis: ThesisData,
  out: CoachSuggestion[],
  dismissed: Set<string>,
  _refMap?: Map<string, any>   // new optional param
...
export function checkC04_SingleChapterFoundational(
  thesis: ThesisData,
  out: CoachSuggestion[],
  dismissed: Set<string>,
  refMap: Map<string, typeof thesis.references[number]>
...
export function checkC05_SelfCitationCluster(
  thesis: ThesisData,
  out: CoachSuggestion[],
  dismissed: Set<string>,
  refMap: Map<string, typeof thesis.references[number]>
...
export function checkL01_FirstPersonSingular(
  thesis: ThesisData,
  out: CoachSuggestion[],
  dismissed: Set<string>,
  _refMap?: Map<string, any>   // new optional param
...
export function checkL02_HedgingOverload(
  thesis: ThesisData,
  out: CoachSuggestion[],
  dismissed: Set<string>,
  _refMap?: Map<string, any>   // new optional param
  ...
  const hedgeWords = [
    ...
    'arguably', 'presumably', 'tentatively', 'potentially',
...
export function checkL03_WeakNominalizations(
  thesis: ThesisData,
  out: CoachSuggestion[],
  dismissed: Set<string>,
  _refMap?: Map<string, any>   // new optional param
...
export function checkL04_VagueQuantifiers(
  thesis: ThesisData,
  out: CoachSuggestion[],
  dismissed: Set<string>,
  _refMap?: Map<string, any>   // new optional param
  ...
  const vagueQuantifiers = [
    ...
    'numerous', 'certain', 'considerable', 'substantial', 'significant',
  ];
  ...
  for (const ch of thesis.chapters) {
    ...
    for (let sIdx = 0; sIdx < sentences.length; sIdx++) {
      ...
      // Check if any vague quantifier appears without quantification (not preceded by a number or "a")
      let matchedQuantifier = '';
      for (const vq of vagueQuantifiers) {
        const regex = new RegExp(`(?:^|\\s)${vq}(?:\\s|$|[,.])`, 'i');
        if (!regex.test(lower)) continue;
        const vqIndex = lower.indexOf(vq.toLowerCase());
        if (vqIndex < 0) continue;
        const before = lower.substring(Math.max(0, vqIndex - 5), vqIndex).trim();
        if (/(?:^|\s)\d+\s*$/.test(before)) continue; // preceded by a number
        if (/(?:^|\s)a\s*$/.test(before)) continue; // preceded by "a"
        matchedQuantifier = vq;
        break;
      }
      if (!matchedQuantifier) continue;
      ...
      if (!hasCite) {
        ...
        out.push({
          ...
          detail: `The word "${matchedQuantifier}" appears without a supporting citation within the following 2 sentences. Vague quantifiers should be backed by specific references.`,
...
export function checkL05_TransitionOveruse(
  thesis: ThesisData,
  out: CoachSuggestion[],
  dismissed: Set<string>,
  _refMap?: Map<string, any>   // new optional param
...
export function checkAS01_AbstractElements(
  thesis: ThesisData,
  out: CoachSuggestion[],
  dismissed: Set<string>,
  _refMap?: Map<string, any>   // new optional param
  ...
  const requiredElements: Array<{ name: string; patterns: string[] }> = [
    { name: 'background/problem context', patterns: ['this study', 'this research', 'this paper', 'this thesis', 'background', 'context', 'recent years', 'field of'] },
    { name: 'objective/research question', patterns: ['objective', 'research question', 'aim', 'purpose', 'goal', 'investigate', 'examine', 'explore'] },
    { name: 'method/approach', patterns: ['method', 'approach', 'technique', 'procedure', 'we propose', 'we present', 'we use', 'using a'] },
    { name: 'results/findings', patterns: ['result', 'finding', 'outcome', 'show that', 'found that', 'achieve', 'improve', 'perform'] },
    { name: 'conclusion/implications', patterns: ['implication', 'contribution', 'significance', 'importance', 'conclude', 'suggest that'] },
  ];
...
export function checkAS02_KeywordsNotInAbstract(
  thesis: ThesisData,
  out: CoachSuggestion[],
  dismissed: Set<string>,
  _refMap?: Map<string, any>   // new optional param
...
export function checkAS03_AcronymBeforeDefinition(
  thesis: ThesisData,
  out: CoachSuggestion[],
  dismissed: Set<string>,
  _refMap?: Map<string, any>   // new optional param
...
export function checkAS04_NumbersBelowTen(
  thesis: ThesisData,
  out: CoachSuggestion[],
  dismissed: Set<string>,
  _refMap?: Map<string, any>   // new optional param
  ...
  for (const ch of thesis.chapters) {
    ...
    const noCitations = noTables.replace(/\\cite(?:p|t|author|year|alp|num)?\{[^}]*\}/g, '');

    // Remove figure references (Figure X, Fig. X) and table references (Table X)
    const noFigRefs = noCitations.replace(/(?:Figure|Fig\.?|Table)\s+\d+/gi, '');
...
export function checkAS05_InconsistentTense(
  thesis: ThesisData,
  out: CoachSuggestion[],
  dismissed: Set<string>,
  _refMap?: Map<string, any>   // new optional param
  ...
  // Compute dominant tense for each chapter
  type ChapterTense = { id: string; title: string; pastCount: number; presentCount: number; dominant: 'past' | 'present' | 'none' };
  const chapterTenses: ChapterTense[] = [];
  ...
  for (const ch of thesis.chapters) {
    ...
    if (total < 3) continue;
    const dominant: ChapterTense['dominant'] = pastCount > presentCount ? 'past' : presentCount > pastCount ? 'present' : 'none';
    chapterTenses.push({ id: ch.id, title: ch.title, pastCount, presentCount, dominant });
  }

  if (chapterTenses.length === 0) return;

  // Compute overall document dominant tense
  const docPastTotal = chapterTenses.reduce((sum, ct) => sum + ct.pastCount, 0);
  const docPresentTotal = chapterTenses.reduce((sum, ct) => sum + ct.presentCount, 0);
  const docTotal = docPastTotal + docPresentTotal;
  if (docTotal < 10) return;

  const docDominant = docPastTotal > docPresentTotal ? 'past' : 'present';

  // Flag chapters whose dominant tense differs from the document dominant tense
  for (const ct of chapterTenses) {
    if (ct.dominant === 'none' || ct.dominant === docDominant) continue;

    const ctTotal = ct.pastCount + ct.presentCount;
    const ctPastRatio = ct.pastCount / ctTotal;
    const ctPresentRatio = ct.presentCount / ctTotal;

    // Only flag if the chapter's non-dominant ratio is significant (> 40%)
    if (ct.dominant === 'past' && ctPastRatio < 0.4) continue;
    if (ct.dominant === 'present' && ctPresentRatio < 0.4) continue;

    const dk = dismissedKey('AS05', ct.id);
    if (dismissed.has(dk)) continue;
    ...
    out.push({
      ...
      chapterId: ct.id,
      location: 'body',
      headline: `Tense shift in "${ct.title}" vs. overall document`,
      detail: `"${ct.title}" uses predominantly ${ct.dominant} tense (${Math.round((ct.dominant === 'past' ? ctPastRatio : ctPresentRatio) * 100)}%) while the overall document uses ${docDominant} tense. Cross-chapter tense inconsistency can confuse readers.`,
      suggestedAction: `Align "${ct.title}" with the document's dominant ${docDominant} tense. ${docDominant === 'present' ? 'Use present tense for discussing current positions and past tense only for specific findings.' : 'Use past tense consistently for reporting study findings and methods.'}`,
      exampleText: docDominant === 'present'
        ? 'Present: "Smith argues that X is true. Jones demonstrates…"'
        : 'Past: "Smith argued that X was true. Jones demonstrated…"',
    });
    break; // One suggestion is enough for cross-chapter check
  }

  // Also check within-paragraph consistency for the literature review
  const litReview = thesis.chapters.find(ch =>
    chapterMatches(ch, /literature|review|related work|state of the art/i)
  );
  if (litReview) {
    const litTense = chapterTenses.find(ct => ct.id === litReview.id);
    if (litTense) {
      const litTotal = litTense.pastCount + litTense.presentCount;
      if (litTotal >= 5) {
        const pastRatio = litTense.pastCount / litTotal;
        const presentRatio = litTense.presentCount / litTotal;
...
export function calculateCoachScore(suggestions: CoachSuggestion[], category?: string): number {
  const filtered = category ? suggestions.filter(s => s.category === category) : suggestions;
  let critical = filtered.filter(s => s.severity === 'critical').length;
  let major = filtered.filter(s => s.severity === 'major').length;
  let minor = filtered.filter(s => s.severity === 'minor').length;
  return Math.max(0, 100 - critical * 15 - major * 5 - minor * 1);
}
...
export function runWritingCoach(
  ...
  return {
    suggestions: capped,
    coachScore: calculateCoachScore(capped),
    scoreBreakdown: {
      structure: calculateCoachScore(capped, 'structure'),
      argument: calculateCoachScore(capped, 'argument'),
      citation: calculateCoachScore(capped, 'citation'),
      language: calculateCoachScore(capped, 'language'),
      academicStyle: calculateCoachScore(capped, 'academic-style'),
    },
