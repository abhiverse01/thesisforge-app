 ...
 */
// ============================================================
// Velocity Report — Single aggregated view for UI display
// ============================================================
...
export interface VelocityReport {
  ...
  status: 'high' | 'moderate' | 'low' | 'stalled';
  streak: number;
  estimatedDaysToComplete: number | null;
}

/**
 * Compute a velocity report for display in the intelligence panel.
 * Uses the same raw velocity data but returns a single summary.
 *
 * @param events - All memory events (filtered internally)
 * @param target - Target word count for the thesis type
 * @param current - Current total word count
 */
export function computeVelocityReport(
  ...
  const velocity = computeVelocity(events, 7); // Last 7 days
  const today = new Date().toDateString();

  const todayEntry = velocity.find(v => v.date === new Date().toISOString().slice(0, 10));
  const wordsToday = todayEntry?.netWords ?? 0;
  const wordsThisWeek = velocity.reduce((s, v) => s + Math.max(0, v.netWords), 0);

  const activeDays = velocity.filter(v => v.netWords > 0).length;
  const avgWordsPerDay = activeDays > 0 ? Math.round(wordsThisWeek / activeDays) : 0;

  // Streak: consecutive days from today backward with > 50 net words
  let streak = 0;
  const dateCheck = new Date();
  for (let i = 0; i < 30; i++) {
    const dayStr = dateCheck.toISOString().slice(0, 10);
    const entry = velocity.find(v => v.date === dayStr);
    if (entry && entry.netWords > 50) {
      streak++;
  ...
  const status: VelocityReport['status'] =
    avgWordsPerDay >= 500 ? 'high' :
    avgWordsPerDay >= 100 ? 'moderate' :
    avgWordsPerDay >= 10  ? 'low' :
    'stalled';

  const remaining = Math.max(0, target - current);
  const estimatedDaysToComplete = avgWordsPerDay > 0
    ? Math.ceil(remaining / avgWordsPerDay)
    : null;

  return { wordsToday, wordsThisWeek, avgWordsPerDay, status, streak, estimatedDaysToComplete };
}
