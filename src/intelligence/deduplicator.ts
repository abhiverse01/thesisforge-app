 ...
 * Handles: https://doi.org/, http://doi.org/, https://dx.doi.org/, bare 10.xxxx/ prefixes.
 */
...
export function detectDuplicatesWithMerge(
  ...
  for (let i = 0; i < references.length; i++) {
    for (let j = i + 1; j < references.length; j++) {
      ...
      // Use normalized DOIs for comparison
      const normDoiA = normalizeDoi(a.doi || '');
      const normDoiB = normalizeDoi(b.doi || '');
      if (normDoiA && normDoiB && normDoiA === normDoiB) {
