...
export interface PackageEntry {
  ...
  conditional?: 'hasCode' | 'hasMath' | 'isReport' | 'isTwoSided' | 'isNotConference';
}
...
const PACKAGE_MANIFEST: PackageEntry[] = [
  ...
  { pkg: 'textcomp',    opts: [],                          purpose: 'Text companion symbols (\\textdegree, \\textquotesingle, etc.)' },

  // ── Page geometry (must load after encoding, before babel) ────
  ...
  { pkg: 'amsthm',      opts: [],                          purpose: 'Theorem environments' },
  ...
  // ── Bibliography ─────────────────────────────────────────────
  // natbib conflicts with IEEEtran (conference template) — use conditional
  { pkg: 'natbib',      opts: ['round', 'colon', 'authoryear'], purpose: 'Author-year citations', conditional: 'isNotConference' },
  ...
  // cleveref conflicts with IEEEtran — use conditional
  { pkg: 'cleveref',    opts: ['nameinlink', 'noabbrev'], purpose: 'Smart cross-refs — MUST LOAD AFTER hyperref', conditional: 'isNotConference' },
];
...
export function detectMathContent(chapters: ThesisData['chapters']): boolean {
  return chapters.some(ch =>
    ch.content && (
      ...
      // Greek letters as math symbols — require math delimiters or command context
      // to avoid false positives from common English words ("alpha testing", "Six Sigma")
      /\\(alpha|beta|gamma|delta|sigma|pi|theta|lambda|mu)\b/.test(ch.content)
    )
  );
}
...
export function resolvePackages(data: ThesisData): ResolvedPackage[] {
  ...
  const isNotConference = data.type !== 'conference';
  ...
  if (data.type === 'phd' && data.options.includeGlossary) {
    extraPackages.push(
      ...
      { name: 'glossaries', options: ['nomain', 'nonumberlist', 'nopostdot'], purpose: 'Glossary support (PhD requirement, noidx for Overleaf)' },
    );
  }
  ...
  const filtered = PACKAGE_MANIFEST
    .filter(entry => {
      ...
      if (entry.conditional === 'isNotConference' && !isNotConference) return false;
      return true;
    });
  ...
  // Combine with extra packages — but keep hyperref/cleveref last.
  // Extra packages (listings, nomencl, glossaries, chngcntr) must be
  // inserted BEFORE hyperref so the load order requirement is met:
  //   ... other packages ... -> extra packages -> hyperref -> cleveref
  const hyperref = manifestPackages.find(p => p.name === 'hyperref');
  const cleveref = manifestPackages.find(p => p.name === 'cleveref');
  const beforeHyperref = manifestPackages.filter(
    p => p.name !== 'hyperref' && p.name !== 'cleveref'
  );

  const allPackages = [...beforeHyperref, ...extraPackages];
  if (hyperref) allPackages.push(hyperref);
  if (cleveref) allPackages.push(cleveref);
...
${bindingOffset}  headheight=15pt,
  ...
  const escapedTitle = escapeLatexMeta(m.title || '');
  const escapedAuthor = escapeLatexMeta(m.author || '');
  const escapedKeywords = escapeLatexMeta(data.keywords.join(', ') || '');
...
\\fancypagestyle{plain}{
  ...
  // ── Cleveref language ────────────────────────────────────────
  // NOTE: cleveref is NOT loaded for conference templates (IEEEtran conflict).
  // Guard \crefname commands to prevent "undefined control sequence" errors.
...
\\crefname{appendix}{Appendix}{Appendices}`);
  ...
  // ── Custom commands ──────────────────────────────────────────
  // Use \providecommand to avoid redefinition errors if a document class
  // or package already defines these commands.
  lines.push(`% ── Custom commands ──────────────────────────────────`);
  lines.push(`\\providecommand{\\thesis}{${escapedTitle}}`);
  lines.push(`\\providecommand{\\thesisauthor}{${escapedAuthor}}`);
  lines.push(`\\providecommand{\\theyear}{${new Date().getFullYear().toString()}}`);
  if (m.university) {
    const escapedInst = escapeLatexMeta(m.university);
    lines.push(`\\providecommand{\\theinstitution}{${escapedInst}}`);
  }
  lines.push(`\\providecommand{\\latex}{\\LaTeX\\xspace}`);
  lines.push(`\\providecommand{\\tex}{\\TeX\\xspace}`);

  // ── Theorem environments ─────────────────────────────────
  // FIXELEVEN: Removed \newtheorem definitions that conflict with amsthm package.
  // amsthm (loaded when hasMath=true) already defines theorem, lemma, corollary,
  // and definition environments. Defining them again causes "Command \theorem
  // already defined" LaTeX compilation error. amsthm does NOT define example
  // or remark, so those are kept.
  lines.push(`% ── Theorem environments (amsthm provides theorem, lemma, corollary, definition) ─────────────`);
  lines.push(`\\makeatletter`);
  lines.push(`\\@ifundefined{theorem}{\\newtheorem{theorem}{Theorem}[chapter]}{}`);
  lines.push(`\\@ifundefined{lemma}{\\newtheorem{lemma}[theorem]{Lemma}}{}`);
  lines.push(`\\@ifundefined{corollary}{\\newtheorem{corollary}[theorem]{Corollary}}{}`);
  lines.push(`\\@ifundefined{definition}{\\newtheorem{definition}[theorem]{Definition}}{}`);
  lines.push(`\\@ifundefined{example}{\\newtheorem{example}{Example}[chapter]}{}`);
  lines.push(`\\@ifundefined{remark}{\\newtheorem{remark}{Remark}[chapter]}{}`);
  ...
  // ── Glossary/Nomenclature setup (PhD only) ─────────────────
  // NOTE: \makenoidxglossary works with single-pass pdflatex on Overleaf.
  // \printglossaries requires external `makeglossaries` tool — NOT available on Overleaf.
  if (data.type === 'phd' && data.options.includeGlossary) {
    lines.push(`% ── Glossary/Nomenclature setup ────────────────────────`);
    lines.push(`\\makenoidxglossary`);
    lines.push(`\\makenomenclature`);
  }
